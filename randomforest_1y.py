...
def predict_future(model, stock_df, future_days=7):
    """
    预测未来数据
    """
    future_data = stock_df.iloc[-1:].copy()
    future_predictions = []
    ...
    # 生成未来日期
    future_dates = pd.date_range(stock_df["日期"].iloc[-1] + timedelta(days=1), periods=future_days, freq="D")
...
def plot_results(stock_df, future_df):
    """
    可视化历史数据、历史预测和未来预测
    """
    plt.figure(figsize=(14, 7))
    plt.plot(stock_df["日期"], stock_df["收盘"], label="历史收盘价", color="blue")
    plt.plot(stock_df["日期"], stock_df["历史预测价格"], label="历史预测价格", color="green", linestyle="dashed")
    plt.plot(future_df["日期"], future_df["预测收盘价"], label="未来预测收盘价", color="red", linestyle="dashed",
             marker="o")
    plt.xlabel("日期")
    plt.ylabel("收盘价")
    plt.title("赛力斯股价历史、历史预测及未来一周预测")
    plt.legend()
    plt.grid()
    plt.show()


def randomforest_1y(stock_code="601127", start_date=None, end_date=None, n_estimators=200, random_state=42,
                    future_days=7):
    """
    主函数：使用过去一年的数据训练随机森林模型并预测未来
    """
    # 获取最近一年的数据
    ...
    # 定义特征和目标变量
    X = stock_df[["MA5", "MA10", "MA20", "成交量", "Lag1", "Lag2", "Lag3"]]
    y = stock_df["收盘"]
    ...
    stock_df.loc[X_train.index, "历史预测价格"] = y_train_pred
    stock_df.loc[X_test.index, "历史预测价格"] = y_test_pred

    # 预测未来数据
    future_df = predict_future(model, stock_df, future_days=future_days)
    ...
    # 可视化结果
    plot_results(stock_df, future_df)
